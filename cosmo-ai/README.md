...
COSMO_DB_PATH=/data/cosmo/db/cosmo.db

HF_TOKEN=your_huggingface_token
HF_DATASET_REPO=your-user/cosmo-trained-data
...
- `COSMO_POWER_PROFILE=low-power|balanced|performance`
- `LOCAL_MODEL_THREADS=<int>`
- `COSMO_EAGER_KNOWLEDGE_BASE_ENABLED=true|false`
- `COSMO_STARTUP_VERIFICATION_ENABLED=true|false`
- `COSMO_PERSIST_MODELS_DIR=true|false`
- `COSMO_WARM_CHAT_RUNTIME_ENABLED=true|false`
- `COSMO_HF_KEEPALIVE_ENABLED=true|false`
- `COSMO_AUTO_COLLECTION_ENABLED=true|false`
- `COSMO_AUTO_RESEARCH_ENABLED=true|false`
- `COSMO_AUTO_TRAINING_ENABLED=true|false`
- `COSMO_BOOTSTRAP_APPROVED_MODELS=true|false`
- `COSMO_BOOTSTRAP_GGUF_RUNTIME=true|false`
- `COSMO_BOOTSTRAP_GGUF_DOWNLOAD=true|false`
- `COSMO_BOOTSTRAP_GGUF_INSTALL=true|false`
...
- `LOCAL_BITNET_MODEL_PATH=/data/cosmo/models/llm/bitnet-cpu/ggml-model-i2_s.gguf`
- `LOCAL_BITNET_REPO_PATH=/data/cosmo/models/llm/bitnet.cpp`
...
- `COSMO_POWER_PROFILE=low-power`
- `COSMO_DATA_ROOT=/data/cosmo`
- `COSMO_DB_PATH=/data/cosmo/db/cosmo.db`
- `COSMO_MODELS_DIR=/data/cosmo/models`
- `COSMO_UPLOADS_DIR=/data/cosmo/uploads`
- `PYTHONUSERBASE=/data/cosmo/runtime/python-user-base`
- `COSMO_EAGER_KNOWLEDGE_BASE_ENABLED=false`
- `COSMO_STARTUP_VERIFICATION_ENABLED=false`
- `COSMO_PERSIST_MODELS_DIR=false`
- `COSMO_WARM_CHAT_RUNTIME_ENABLED=false`
- `COSMO_BOOTSTRAP_GGUF_RUNTIME=false`
- `COSMO_BOOTSTRAP_APPROVED_MODELS=false`

If persistent storage is attached, local downloads and generated files can live there.
If `/data` is not writable in the runtime, Cosmo now falls back automatically
to a writable local path such as `data/` or `/tmp/cosmo` instead of crashing
on startup.
...
`.venv`, `venv`, `venv312`, or a custom path supplied via `COSMO_VENV`.
...
- `drizzle/0000_cosmo_initial.sql` as the bootstrap schema the Python app also applies
...
When that secret is present and `COSMO_TEST_MODE=true`, the backend accepts
...
- `COSMO_ENABLE_WEB_KNOWLEDGE_FALLBACK=true|false`
- `COSMO_WEB_FALLBACK_MIN_CONTEXT_CHARS=160`
- `COSMO_WEB_FALLBACK_PROVIDER=auto|cloudflare|legacy`
- `COSMO_WEB_FALLBACK_MAX_PAGES=2`
- `COSMO_WEB_FALLBACK_MAX_SITES=1`
- `COSMO_WEB_FALLBACK_DEPTH=1`
- `COSMO_WEB_FALLBACK_RENDER=false`
- `COSMO_WEB_FALLBACK_REFRESH_EXISTING=false`
...
- `COSMO_SELF_LEARNER_MIN_STEPS=50`
- `COSMO_SELF_LEARNER_MIN_SEQUENCES=20`
...
- `COSMO_EMBEDDER_MODEL=sentence-transformers/all-MiniLM-L6-v2`
- `COSMO_EMBEDDER_DEVICE=cpu`
- `COSMO_EMBEDDER_BATCH_SIZE=8`
- `COSMO_EMBEDDER_MAX_SEQ_LENGTH=256`
- `COSMO_EMBEDDER_MAX_CHARS=1600`
- `COSMO_EMBEDDER_CACHE_SIZE=512`

### GitHub Actions Deployment

The repo includes a GitHub Actions workflow at
`/.github/workflows/deploy-hf-space.yml` that deploys the `cosmo-ai`
...
- Managed model-directory persistence is now opt-in via `COSMO_PERSIST_MODELS_DIR=true`.
- If you want the Space to pre-warm the chat runtime or background-bootstrap models after startup, re-enable `COSMO_WARM_CHAT_RUNTIME_ENABLED`, `COSMO_BOOTSTRAP_GGUF_RUNTIME`, or `COSMO_BOOTSTRAP_APPROVED_MODELS` in Space settings.
- Background GGUF installs use `PYTHONUSERBASE=/data/cosmo/runtime/python-user-base`, so attached persistent storage can keep the compiled runtime between restarts.
