Cosmo AI - Optimized Training Script
...
from model.tokenizer import TOKENIZER_BACKEND_BYTELEVEL, TokenizerConfig, CosmoTokenizer
...
DEFAULT_RANDOM_WIKIPEDIA_PAGES = max(0, int(os.getenv("COSMO_DYNAMIC_RANDOM_WIKIPEDIA_PAGES", "4")))
DEFAULT_RECENT_RESEARCH_DOCS = max(0, int(os.getenv("COSMO_DYNAMIC_RESEARCH_DOCS", "10")))
DEFAULT_GUTENBERG_DISCOVERIES = max(0, int(os.getenv("COSMO_DYNAMIC_GUTENBERG_DISCOVERIES", "4")))
DEFAULT_ARXIV_DISCOVERIES = max(0, int(os.getenv("COSMO_DYNAMIC_ARXIV_DISCOVERIES", "4")))
DEFAULT_DYNAMIC_WEB_FETCH_LIMIT = max(1, int(os.getenv("COSMO_DYNAMIC_WEB_FETCH_LIMIT", "12")))
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train Cosmo AI (Optimized)")
    ...
    logger.info("=== Cosmo AI Training (Optimized) ===")
    ...
    if tokenizer_path.exists():
        ...
        if args.rebuild_tokenizer or not tokenizer.is_modern_backend():
            tokenizer = CosmoTokenizer(TokenizerConfig(
