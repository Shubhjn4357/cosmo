...
async def launch_business_session(request: LaunchSessionRequest) -> Dict[str, str]:
    ...
    session = await launch_session(goal=request.goal, company_context=request.company_context)
...
async def get_business_diagnostics() -> Dict[str, Any]:
    ...
    from services.cosmo_business import get_business_engine
    engine = get_business_engine()
    return await engine.run_hardware_diagnostics()
...
async def start_handoff(session_id: str, request: HandoffRequest):
    ...
    session = BusinessSession(id=session_id, goal=data['goal'], company_context=data['company_context'])
...
async def cast_vote(session_id: str, request: VoteRequest):
    ...
    session.consensus_votes = data.get("consensus_votes", {})
    session.multi_user_mode = data.get("multi_user_mode", True)
    session.is_handoff_active = data.get("is_handoff_active", True)
    session.messages = data.get("messages", [])
    session.tasks = [
        BusinessTask(
            id=t.get('id', ''),
            ...
            assigned_to=EmployeeRole(t.get('assigned_to', 'analyst')),
