"""
Cosmo AI - Email Notification Service
...
FROM_EMAIL = os.getenv("FROM_EMAIL", SMTP_USER or "noreply@cosmo-ai.com")
...
def send_admin_alert(subject: str, message: str, details: Optional[dict[Any, Any]] = None) -> bool:
    """
    Send alert to admin
    ...
    <html>
      <body>
        <h2 style="color: #dc3545;">Cosmo AI Admin Alert</h2>
        <p><strong>{message}</strong></p>
        {f'<h3>Details:</h3><ul>' + ''.join([f'<li><strong>{k}:</strong> {v}</li>' for k, v in details.items()]) + '</ul>' if details else ''}
        <hr>
        <p style="color: #6c757d; font-size: 12px;">
          This is an automated alert from Cosmo AI security system.
        </p>
      </body>
    </html>
    """
   
    return send_email(ADMIN_EMAIL, f"[Cosmo AI Alert] {subject}", body_text, body_html)
