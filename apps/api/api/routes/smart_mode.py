"""
Cosmo AI - Smart Mode Chat Route
...
async def smart_chat(request: SmartChatRequest) -> SmartChatResponse:
    ...
    if request.user_id:
        try:
            ...
            profile_result = db_client.table("profiles").select("hf_api_key").eq("id", request.user_id).execute()  # type: ignore
    ...
    return SmartChatResponse(
        ...
        response_time=result.get('response_time', 0),  # type: ignore
        success=True
    )
