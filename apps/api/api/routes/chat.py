...
DEFAULT_SYSTEM_PROMPT = (
    "You are Cosmo AI, a direct assistant for chat, coding, roleplay, and research. "
    "Give accurate, useful answers with minimal filler. "
    "STRICT PRIVACY: Do not output Personal Identifying Information (PII) like emails, full names, or phone numbers. "
    "If providing code samples, use generic placeholders for sensitive data. "
...
class MultimodalResponse(TypedDict):
    has_image: bool
...
def _copy_chat_request(request: ChatRequest, **updates: Union[str, int, float, bool, list, dict]) -> ChatRequest:
...
def _self_learner_text_fallback_enabled() -> bool:
    return os.getenv("COSMO_SELF_LEARNER_TEXT_FALLBACK", "true").lower() == "true"
...
class MultimodalContext(TypedDict):
    has_image: bool
    context: str
    matches: list[dict[str, Union[str, float, dict]]]
    preview_url: Optional[str]


async def _prepare_self_learner_multimodal_context(request: ChatRequest) -> MultimodalContext:
...
async def _generate_self_learner_visual(request: ChatRequest) -> dict[str, Any]:  # type: ignore
    from model.hybrid_vision import get_hybrid_model
    ...
    if not generated_image and os.getenv("COSMO_SELF_LEARNER_IMAGE_FALLBACK_LOCAL", "true").lower() == "true":
...
def _build_multimodal_fallback_text(
    *,
    learner_state: dict[str, Any],  # type: ignore
    min_steps: int,
    min_sequences: int,
    image_context: dict[str, Any],  # type: ignore
    visual_result: Optional[dict[str, Any]],  # type: ignore
) -> str:
    lines = [
...
def _build_prompt(
    ...
    *,
    ...
    knowledge_sources: Optional[list[dict[str, Any]]] = None,  # type: ignore
) -> tuple[str, list[dict]]:
...
async def _resolve_knowledge_context(request: ChatRequest, state) -> tuple[str, list[dict]]:
    ...
    min_context_chars = max(0, int(os.getenv("COSMO_WEB_FALLBACK_MIN_CONTEXT_CHARS", "160")))
    if len((context or "").strip()) >= min_context_chars:
        return context, sources

    web_fallback_enabled = os.getenv("COSMO_ENABLE_WEB_KNOWLEDGE_FALLBACK", "true").lower() == "true"
    ...
    try:
        ...
        fallback_request = DiscoverRequest(
            ...
            max_pages=max(1, int(os.getenv("COSMO_WEB_FALLBACK_MAX_PAGES", "2"))),
            provider=os.getenv("COSMO_WEB_FALLBACK_PROVIDER", "auto"),
            max_sites=max(1, int(os.getenv("COSMO_WEB_FALLBACK_MAX_SITES", "1"))),
            depth=max(1, int(os.getenv("COSMO_WEB_FALLBACK_DEPTH", "1"))),
            render=os.getenv("COSMO_WEB_FALLBACK_RENDER", "false").lower() == "true",
            refresh_existing=os.getenv("COSMO_WEB_FALLBACK_REFRESH_EXISTING", "false").lower() == "true",
        )
...
def _record_chat_interaction(
    ...
    *,
    ...
    # Audit: Anonymize both prompt and response before recording
    scrubbed_prompt = anonymize_lesson(request.message)
    scrubbed_response = anonymize_lesson(response_text)
...
async def chat(request: ChatRequest) -> ChatResponse:
    ...
    if not token_result.get('success'):
        raise HTTPException(
            status_code=403 if 'insufficient' in token_result.get('error', '') else 401,  # type: ignore
            detail=token_result
        )
    
    state = get_app_state()
    
    if (request.message.lower().startswith("cosmo") or 
        os.getenv("COSMO_DEFAULT_MODEL_MODE") == "cosmo"):
        from services.cosmo_model import cosmo_instance
        logger.info("Routing request to Cosmo Unified Model")
        response_text = await cosmo_instance.generate_response(request.message, history=request.history)  # type: ignore
        
        # Audit: Scrub unified response
        scrubbed_text = anonymize_lesson(response_text)

        return ChatResponse(
            response=scrubbed_text,
            tokens_used=max(1, len(scrubbed_text.split())),
            sources=[],
            model_used="cosmo-1.1",
            backend="mythos_unified"
        )
...
async def chat_self_learner(request: ChatRequest) -> ChatResponse:
    """
    Generate a response using the built-in scratch Cosmo transformer.
    """
    from api.route import get_app_state
    from api.routes.profile import get_db_client
    ...
    augmented_request = _copy_chat_request(
        request,
        context="\n\n".join(  # type: ignore
            part for part in (request.context, multimodal_context) if str(part or "").strip()  # type: ignore
        ).strip() or None,
    )
        ...
        if image_context.get("has_image") or (visual_result and visual_result.get("generated_image")):
            return ChatResponse(
                response=_build_multimodal_fallback_text(
                    ...
                    image_context=image_context,  # type: ignore
                    visual_result=visual_result,
                ),
                tokens_used=0,
                sources=[],
                model_used="cosmo-micro-transformer",
    ...
    try:
        ...
        _record_chat_interaction(request, response_text, result, sources, learning_source="self-learner")
...
@router.post("/chat/cosmo")
async def chat_cosmo(request: ChatRequest) -> ChatResponse:
    from services.cosmo_model import cosmo_instance
    from api.routes.profile import get_db_client
    from services.token_service import check_and_use_tokens
...
async def chat_stream(request: ChatRequest):
    ...
    async def generate():
        ...
        try:
            knowledge_context, knowledge_sources = await _resolve_knowledge_context(request, state)
            prompt, _ = _build_prompt(request, state, knowledge_context=knowledge_context, knowledge_sources=knowledge_sources)
    ...
    try:
        ...
        for pair in request.pairs:
            if "input" in pair and "output" in pair:
                save_training_pair(pair["input"], pair["output"], source=pair.get("model", "mobile"))
                count += 1
