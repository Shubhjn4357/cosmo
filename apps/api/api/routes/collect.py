...
async def collect_text_from_sources(count: int = 10) -> List[Dict]:
    ...
    for url in urls_to_try:
        ...
        if result and result.get("type") == "text":
            # Audit: Scrub public text before collection
            raw_text = result.get("text") or ""
            result["text"] = anonymize_lesson(raw_text)
            collected.append(result)
...
def _local_text_representation(image_data: Dict) -> str:
    ...
    raw_rep = f"{title}. Imported from {source} for local vision learning."
    # Audit: Scrub local representation
    return anonymize_lesson(raw_rep)
...
async def encode_image_for_learning(image_data: Dict) -> Optional[Dict]:
    ...
    collection_stats["encoded_locally"] += 1  # type: ignore
    if remote_mirror and remote_mirror.get("success"):
        collection_stats["remote_mirrors"] += 1  # type: ignore
...
async def _run_collection(request: DataCollectionRequest) -> Dict:
    ...
    if request.source_type in {"text", "both"}:
        ...
        results["text"] = texts  # type: ignore
        collection_stats["text_collected"] += len(texts)  # type: ignore

    if request.source_type in {"images", "both"}:
        images = await collect_images_from_sources(min(request.count, 20))
        results["images"] = images  # type: ignore
        collection_stats["images_collected"] += len(images)  # type: ignore

        if request.auto_feed:
            for image_data in images[: request.count]:
                encoded = await encode_image_for_learning(image_data)
                if encoded:
                    results["encoded"].append(  # type: ignore
                        {
                            "source": image_data["source"],
                            "title": image_data["title"],
                            "dimension": encoded["dimension"],
                            "sent_to_cosmo": encoded["sent_to_cosmo"],
    ...
    collection_stats["last_collection"] = datetime.now().isoformat()  # type: ignore
    collection_stats["last_backend"] = _collection_backend()  # type: ignore
