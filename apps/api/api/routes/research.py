...
async def _discover_with_legacy_scraper(
    ...
    if os.getenv("COSMO_TEST_MODE", "false").lower() == "true":
        ...
        allowed_domains = sorted({decision.get("domain") for decision in SOURCE_POLICY.filter_urls(urls)[1] if decision.get("allowed") and decision.get("domain")})  # type: ignore
        scraper = get_scraper()
...
async def _run_autonomous_research_cycle(source_id: Optional[str] = None) -> dict[str, Any]:
    ...
    AUTO_RESEARCH_RUNTIME["last_cycle_started_at"] = time.time()  # type: ignore
    ...
    try:
        ...
        AUTO_RESEARCH_RUNTIME["last_error"] = str(exc)  # type: ignore
        ...
        AUTO_RESEARCH_RUNTIME["last_cycle_finished_at"] = time.time()  # type: ignore
