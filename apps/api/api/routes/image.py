"""
Cosmo AI - Local-only image generation routes.
...
TEST_MODE = os.environ.get("COSMO_TEST_MODE", "false").lower() == "true"
...
async def generate_image(request: ImageGenerationRequest) -> ImageGenerationResponse:
    ...
    if not artifact.get("downloaded") or not artifact_path:
        size_str = f" {(artifact.get('size_bytes') or 0)/1000000:.1f}MB"
        raise HTTPException(
            status_code=423,
            detail=f"Image model '{selected_model}' is still actively downloading in the background.{size_str}. Please wait 1-2 minutes.",
        )
...
async def list_models():
    ...
    return {
        ...
        "status": "local_only",
        "mode": "Downloaded checkpoints running on the Cosmo server",
...
async def get_server_info():
    return {
        "server": "Cosmo AI Image Generation",
        "version": "3.1.0",
        "deployment": {
            "target": "Local checkpoint runtime",
            "ram": os.environ.get("COSMO_SERVER_RAM_LABEL", "CPU"),
