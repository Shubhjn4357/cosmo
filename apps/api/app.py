...
def _prebind_delay_seconds() -> float:
    try:
        return max(0.0, float(os.getenv("COSMO_PREBIND_DELAY_SECONDS", "0")))
    except Exception:
        return 0.0
...
if __name__ == "__main__":
    ...
    try:
        ...
        if os.getenv("SPACE_ID") or os.getenv("HUGGINGFACE_SPACES"):
            ...
            os.environ.setdefault("COSMO_POWER_PROFILE", "low-power")
            os.environ.setdefault("COSMO_STARTUP_VERIFICATION_ENABLED", "false")
            os.environ.setdefault("COSMO_EAGER_KNOWLEDGE_BASE_ENABLED", "false")
            os.environ.setdefault("COSMO_WARM_CHAT_RUNTIME_ENABLED", "false")
        ...
        ensure_app_dirs()
        logger.info("Starting Cosmo AI server...")
