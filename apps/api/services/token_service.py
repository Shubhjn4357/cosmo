...
from typing import Optional, Dict, List, Union, TypedDict, cast
...
class TokenResult(TypedDict):
    ...
    used_today: Optional[float]


class UserTokenStatus(TypedDict):
    used: float
    limit: float
    remaining: float
    tier: str
...
class GuestTokenManager:
    ...
    def use_tokens(self, session_id: str, amount: float) -> TokenResult:
        """
...
class TokenService:
    ...
    async def check_and_use_tokens(
        ...
        return {
            ...
            'used': 0.0,
            'is_guest': bool(session_id and not user_id),
            'error': None,
            'current': None,
            'required': None,
            'used_today': 0.0
    ...
    async def get_user_tokens(self, user_id: str) -> UserTokenStatus:
        return {
            'used': 0,
            'limit': float('inf'),
            'remaining': float('inf'),
