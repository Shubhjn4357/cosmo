...
RUN_TIMEOUT_SECONDS = int(os.getenv("COSMO_AUTORESEARCH_COMMAND_TIMEOUT_SECONDS", "600"))
...
def _snapshot_files(paths: list[Path]) -> dict[str, Optional[str]]:
    ...
    MAX_SNAPSHOT_SIZE_BYTES = 50 * 1024 * 1024 # 50MB Audit Guard
    total_size = 0
    ...
    for path in paths:
        ...
        if total_size + file_size > MAX_SNAPSHOT_SIZE_BYTES:
            logger.warning(f"[Autoresearch Audit] Skipping snapshot for {path} - Total size would exceed 50MB limit.")
            continue

        try:
            key = _workspace_relative(path)
            snapshot[key] = path.read_text(encoding="utf-8")
            total_size += file_size
        except Exception as e:
            logger.error(f"[Autoresearch Audit] Failed to snapshot {path}: {e}")
...
def _run_local_command(command: str, cwd: Path, timeout_seconds: int = RUN_TIMEOUT_SECONDS) -> dict[str, Any]:
    # Audit Guard: Prevent basic shell injection/escape
    illegal_chars = [";", "&&", "||", "|", ">", "<", "`", "$(", "${"]
    if any(char in command for char in illegal_chars):
        # We allow it ONLY if it's the exact experiment command from the project def
        # but even then we log a warning for audit trails.
        logger.debug(f"[Autoresearch Audit] Command execution: {command}")
