...
_AIRLLM_IMPORT_CACHE: Optional[dict[str, Any]] = None  # type: ignore
...
def airllm_import_diagnostics(reset_cache: bool = False) -> AirLLMDiagnostics:
    ...
    if _AIRLLM_IMPORT_CACHE is not None and not reset_cache:
        return _AIRLLM_IMPORT_CACHE  # type: ignore
    ...
    if not _package_available("airllm"):
        ...
        _AIRLLM_IMPORT_CACHE = diagnostics  # type: ignore
        return dict(diagnostics)  # type: ignore
    ...
    try:
        ...
        if "BetterTransformer requires transformers<4.49" not in message and "optimum.bettertransformer" not in message:  # type: ignore
            _AIRLLM_IMPORT_CACHE = diagnostics  # type: ignore
            return dict(diagnostics)  # type: ignore

    try:
        _purge_airllm_modules()  # type: ignore
        _install_bettertransformer_shim()  # type: ignore
        diagnostics["module"] = importlib.import_module("airllm")
        diagnostics["available"] = True
...
def _resolve_micro_checkpoint_path(config: RuntimeConfig) -> Optional[Path]:
    ...
    for candidate in fallback_candidates:  # type: ignore
        if candidate.exists():  # type: ignore
            return candidate.resolve()  # type: ignore
...
def validate_runtime_config(
    ...
    *,
    ...
    validation: Dict[str, Any] = {  # type: ignore
...
class ChatRuntimeManager:
    ...
    def readiness(self) -> Dict[str, Any]:  # type: ignore
        ...
        if backend == "stub":
            ...
            if not backend_available:
                ...
                runner_label = runner.get("path") or runner.get("command_template") or "configured runner"  # type: ignore
    ...
    def status(self) -> Dict[str, Any]:  # type: ignore
        resolved = resolve_runtime_choice(self.config)
        return {
            "configured_backend": self.config.backend,
    ...
    def generate(
        ...
        if self._backend_name == "llama_cpp":
            response = self._model(  # type: ignore
                prompt,
        ...
        if self._backend_name == "llama_cpp_cli":
            cli_path = self._model["cli_path"]  # type: ignore
            model_path = self._model["model_path"]  # type: ignore
            command = [
                cli_path,
                "-m",
                model_path,
        ...
        if self._backend_name == "bitnet_cpp":
            runner = self._model["runner"]  # type: ignore
            model_path = self._model["model_path"]  # type: ignore
        ...
        if self._backend_name == "micro_transformer":
            ...
            prompt_ids = self._tokenizer.encode(prompt, add_special_tokens=True)  # type: ignore
            max_prompt_tokens = max(8, self.config.max_context_tokens - max_new_tokens)
            if len(prompt_ids) > max_prompt_tokens:
                prompt_ids = prompt_ids[-max_prompt_tokens:]
                if prompt_ids and prompt_ids[0] != self._tokenizer.bos_id:  # type: ignore
                    prompt_ids = [self._tokenizer.bos_id, *prompt_ids[1:]]  # type: ignore
            ...
            output_ids = self._model.generate(  # type: ignore
                input_ids,
                max_new_tokens=max_new_tokens,
                temperature=max(temperature, 1e-5),
                top_k=50,
                top_p=top_p,
                stop_tokens=[self._tokenizer.eos_id],  # type: ignore
            )
            generated_ids = output_ids[0][len(prompt_ids) :].tolist()
            text = self._tokenizer.decode(generated_ids, skip_special_tokens=True).strip()  # type: ignore
        ...
        if self._backend_name == "airllm":
            ...
            text = self._tokenizer.decode(output_ids[0], skip_special_tokens=True)  # type: ignore
            if text.startswith(prompt):
                text = text[len(prompt):].strip()
            # Failsafe truncation
            for stop_word in stop_words:
