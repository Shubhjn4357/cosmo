...
from typing import Any, Optional, TypedDict, Union, TypeVar, Generic, Sequence
...
class DatabaseValidation(DatabaseStatus):
    reachable: bool
    schema_ready: bool
    missing_tables: list[str]
    table_count: int
    tables_sample: list[str]
    profiles_count: int
    sync_result: str
    validated_at: float
...
class AuthUser:
    ...
    def model_dump(self) -> dict[str, SqlValue]:
        return {
            "id": self.id,
            "email": self.email,
...
class TursoQuery:
    def __init__(self, client: "TursoClient", table_name: str):
        ...
        self.payload: Optional[Union[dict[str, JsonValue], list[dict[str, JsonValue]]]] = None
        self.filters: list[tuple[str, str, SqlValue]] = []
    ...
    def insert(self, values: dict[str, JsonValue] | list[dict[str, JsonValue]]):
        self.operation = "insert"
        self.payload = values
        return self

    def update(self, values: dict[str, JsonValue]):
...
class TursoAuthAdapter:
    ...
    def sign_up(self, payload: dict[str, JsonValue]) -> AuthResponse:
        email = (payload.get("email") or "").strip().lower()  # type: ignore
        ...
        display_name = (
            payload.get("options", {})  # type: ignore
            .get("data", {})
            .get("display_name")
            or email.split("@")[0]
        )
        ...
        self.client.table("profiles").insert(
            {
                "id": user_id,
                "email": email,
                "password_hash": hashlib.sha256(password.encode()).hexdigest(),  # type: ignore
    ...
    def sign_in_with_password(self, payload: dict[str, JsonValue]) -> AuthResponse:
        email = (payload.get("email") or "").strip().lower()  # type: ignore
        password = payload.get("password") or ""
        if not email or not password:
            raise ValueError("Email and password are required")
    ...
    def sign_in_with_id_token(self, payload: dict[str, JsonValue]) -> AuthResponse:
        provider = (payload.get("provider") or "").strip().lower()  # type: ignore
        token = payload.get("token") or payload.get("id_token") or ""
        if provider != "google":
            raise ValueError("Unsupported identity provider")

        claims = verify_google_id_token(token)  # type: ignore
        email = claims["email"]
    ...
    def reset_password_for_email(self, email: str) -> Optional[str]:
...
class TursoClient:
    ...
    def _encode_value(self, table_name: str, column: str, value: JsonValue) -> SqlValue:
        if value == "now()":
            return utc_now()
    ...
    def _decode_row(self, table_name: str, row: Sequence[Any]) -> dict[str, JsonValue]:
        data = dict(row)
    ...
    def _build_where(self, query: TursoQuery) -> tuple[str, list[SqlValue]]:
        clauses: list[str] = []
    ...
    def _insert(self, query: TursoQuery) -> QueryResult:
        ...
        inserted_rows: list[dict[str, JsonValue]] = []

        for original_row in rows:
            row = dict(original_row)  # type: ignore
    ...
    def _update(self, query: TursoQuery) -> QueryResult:
        connection = self._get_connection()
        updates = dict(query.payload or {})  # type: ignore
    ...
    def rpc(self, name: str, payload: dict[str, JsonValue]) -> QueryResult[JsonValue]:
        if name != "use_tokens":
            raise ValueError(f"Unsupported RPC '{name}'")

        user_id = payload.get("p_user_id")
        requested_tokens = float(payload.get("p_tokens") or 0)  # type: ignore
...
def database_runtime_status() -> DatabaseStatus:
    turso_url = os.getenv("TURSO_DATABASE_URL", "").strip()
    return {
...
def validate_database_connection() -> DatabaseValidation:
