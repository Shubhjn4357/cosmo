...
class CosmoModel:
    ...
    async def _tool_image_generate(self, prompt: str) -> str:
        ...
        artifact = resolve_bootstrap_artifact("image", spec.id, spec.filename)  # type: ignore
        out_path = UPLOADS_DIR / "generated" / f"cosmo_{uuid.uuid4().hex}.png"
        out_path.parent.mkdir(parents=True, exist_ok=True)
        
        img = await asyncio.to_thread(
            local_image_runtime.generate,
            model={"id": spec.id},  # type: ignore
    ...
    async def generate_response(self, prompt: str, history: List[Dict[str, str]] = None) -> str:  # type: ignore
        """
