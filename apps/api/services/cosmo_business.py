...
class HardwareDiagnostics(TypedDict):
    filesystem: Dict[str, bool]
    mythos: Dict[str, Union[bool, int]]
...
class CEOAgent:
    ...
    async def plan(self, goal: str, company_context: str) -> tuple[List[BusinessTask], str]:
        raw = await _call_llm(
...
class AutonomousBusinessEngine:
    ...
    async def run(self, session: BusinessSession, on_progress: Optional[Callable[[BusinessSession], None]] = None) -> BusinessSession:
    ...
    async def run_hardware_diagnostics(self) -> HardwareDiagnostics:
        results = {
        ...
        try:
            ...
            results["filesystem"]["writable"] = test_file.read_text() == "ok"  # type: ignore
            test_file.unlink()
        except Exception:
            pass
        ...
        if cosmo_instance.mythos_graph.exists():
            results["mythos"]["graph_exists"] = True  # type: ignore
            try:
                from utils.encryption import EncryptedJSONLReader
                reader = EncryptedJSONLReader(cosmo_instance.mythos_graph)
                results["mythos"]["lesson_count"] = sum(1 for _ in reader)  # type: ignore
            except Exception:
                pass
...
def load_session(session_id: str) -> Optional[Dict[str, Union[str, int, float, List[Dict[str, str]], bool]]]:
...
def is_session_running(session_id: str) -> bool:
    return any(t.status == TaskStatus.RUNNING for t in (load_session(session_id) or {}).get("tasks", []))  # type: ignore
