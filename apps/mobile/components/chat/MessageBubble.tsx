/**
 * Cosmo App - MessageBubble Component
    ...
    const handleDownloadImage = async () => {
        ...
        try {
            ...
            if (imageUrl.startsWith('/')) {
                const apiUrl = process.env.EXPO_PUBLIC_API_BASE_URL || 'https://shubhjn-Cosmo-ai.hf.space';
                imageUrl = `${apiUrl}${imageUrl}`;
            }

            const fileName = `Cosmo_image_${Date.now()}.png`;
            ...
            await MediaLibrary.createAlbumAsync('Cosmo AI', asset, false);
            toast.success('Downloaded', 'Image saved to your gallery in the "Cosmo AI" album');
        } catch (error) {
                                ...
                                <Image
                                    source={{
                                        uri: message.imageUri.startsWith('/')
                                            ? `${process.env.EXPO_PUBLIC_API_BASE_URL || 'https://shubhjn-Cosmo-ai.hf.space'}${message.imageUri}`
                                            : message.imageUri,
                                    }}
                                    style={styles.image}
                                    resizeMode="cover"
