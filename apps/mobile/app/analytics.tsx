...
import { UsageAnalytics, TokenAnalytics, PopularModels, ModelUsage, IconName } from '@/types';
...
export default function AnalyticsScreen() {
    ...
    const [popularModels, setPopularModels] = useState<PopularModels | null>(null);
    ...
    const loadAnalytics = async () => {
        ...
        try {
            ...
            const [usage, tokens, models] = await Promise.all([
                cosmoAPI.getUsageAnalytics({ userId: user.id, period }),
                cosmoAPI.getTokenAnalytics({ userId: user.id, period }),
                cosmoAPI.getPopularModels(),
            ]);

            setUsageData(usage as UsageAnalytics);
            setTokenData(tokens as TokenAnalytics);
            setPopularModels(models as PopularModels);
        } catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            console.error('Failed to load analytics:', error);
            toast.error('Error', `Failed to load analytics data: ${errorMessage}`);
        } finally {
            setLoading(false);
        }
    };
    ...
    const StatCard = ({ 
        ...
        <View style={[styles.statCard, { backgroundColor: theme.colors.surface }]}>
            <View style={[styles.statIcon, { backgroundColor: color + '20' }]}>
                <Ionicons name={icon as IconName} size={24} color={color} />
            </View>
                ...
                {popularModels?.models && (
                    <View style={[styles.section, { backgroundColor: theme.colors.surface }]}>
                        ...
                        <View style={styles.modelsList}>
                            {popularModels.models.slice(0, 5).map((model: ModelUsage, index: number) => (
