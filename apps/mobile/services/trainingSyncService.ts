/**
 * Cosmo App - Training Data Sync Service
...
const TRAINING_DATA_KEY = 'Cosmo_training_data';
const LAST_SYNC_KEY = 'Cosmo_last_training_sync';
const DEVICE_ID_KEY = 'Cosmo_device_id';
...
class TrainingSyncService {
    ...
    async addCorrection(input: string, correctedOutput: string, originalOutput: string, model: string = 'default') {
        ...
        try {
            await cosmoAPI.submitCorrection(input, correctedOutput, originalOutput);
        } catch (error) {
    ...
    async syncToServer(): Promise<{ success: boolean; synced: number }> {
        ...
        try {
            ...
            const result = await cosmoAPI.syncTrainingData(pairs, this.deviceId);
