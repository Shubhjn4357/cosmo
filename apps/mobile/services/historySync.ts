/**
 * Cosmo App - History Sync Service
 * Offline-first sync of chat history with the Cosmo server history API.
 */

import AsyncStorage from '@react-native-async-storage/async-storage';
import NetInfo from '@react-native-community/netinfo';
import { historyAPI, ChatHistory, RawMessage } from './profileAPI';

const OFFLINE_QUEUE_KEY = 'Cosmo_offline_sync_queue';
const LOCAL_HISTORY_KEY = 'Cosmo_local_history';
...
class HistorySyncService {
     ...
     */
    async syncChatCompletion(chatId: string, messages: RawMessage[], title?: string) {
        if (!this.userId) return;
     ...
     */
    private generateTitle(messages: RawMessage[]): string {
