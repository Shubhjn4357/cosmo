 ...
 * Local LLM Service
 * =================
 * Handles local GGUF model inference using llama.rn.
...
type LlamaContextInterface = LlamaContext;

// Dynamic import — guards against web and missing native module.
let initLlama: typeof import('llama.rn').initLlama | null = null;

async function ensureLlama() {
    if (Platform.OS === 'web') return null;
    if (initLlama) return initLlama;
    try {
        const llamaModule = await import('llama.rn');
        initLlama = llamaModule.initLlama;
        return initLlama;
    } catch (error) {
        console.warn('[LocalLLM] llama.rn not available:', error);
        return null;
    }
}
...
class LocalLLMService {
    ...
    async loadCustomModel(): Promise<boolean> {
        ...
        // Platform check
        const llamaInit = await ensureLlama();
        if (!llamaInit) {
            console.warn('Local LLM not supported on this platform');
            return false;
        }
