/**
 * Cosmo App - Data Collection Service
...
const CONSENT_KEY = 'Cosmo_data_collection_consent';
const USAGE_DATA_KEY = 'Cosmo_usage_data';
const LAST_UPLOAD_KEY = 'Cosmo_last_data_upload';
...
class DataCollectionService {
    ...
    async uploadToServer(): Promise<boolean> {
        ...
        try {
            const response = await fetch(`${cosmoAPI.getBaseUrl()}/api/analytics/collect`, {
                method: 'POST',
                headers: {
    ...
    private async getDeviceId(): Promise<string> {
        let deviceId = await AsyncStorage.getItem('Cosmo_analytics_device_id');
        if (!deviceId) {
            deviceId = `anon_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
            await AsyncStorage.setItem('Cosmo_analytics_device_id', deviceId);
        }
        return deviceId;
    }
