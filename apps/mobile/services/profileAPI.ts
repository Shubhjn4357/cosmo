/**
 * Cosmo App - Profile API Service
 * Client-side service that calls the Cosmo server API for auth, profiles,
 * history, and preferences. Persistent storage lives on the server in Turso.
 */

import { cosmoAPI } from './api';
import { UserProfile, AuthSession, RawChatHistory, RawMessage } from '@/types';
export { UserProfile, AuthSession, RawChatHistory as ChatHistory, RawMessage };
...
export const authAPI = {
    ...
    async signIn(email: string, password: string): Promise<{ session: AuthSession | null; error: string | null }> {
        try {
            const response = await fetch(`${cosmoAPI.getBaseUrl()}/api/auth/signin`, {
                method: 'POST',
    ...
    async signUp(email: string, password: string, displayName?: string): Promise<{ success: boolean; session: AuthSession | null; error: string | null }> {
        try {
            const response = await fetch(`${cosmoAPI.getBaseUrl()}/api/auth/signup`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    async signInWithGoogle(idToken: string): Promise<{ session: AuthSession | null; error: string | null }> {
        try {
            const response = await fetch(`${cosmoAPI.getBaseUrl()}/api/auth/google`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    async resetPassword(email: string): Promise<{ error: string | null }> {
        try {
            const response = await fetch(`${cosmoAPI.getBaseUrl()}/api/auth/reset-password`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    async signOut(): Promise<boolean> {
        try {
            const response = await fetch(`${cosmoAPI.getBaseUrl()}/api/auth/signout`, {
                method: 'POST',
            });
            const data = await response.json();
            return data.success;
...
export const profileAPI = {
    ...
    async createProfile(
        ...
        try {
            const response = await fetch(`${cosmoAPI.getBaseUrl()}/api/profile`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    user_id: userId,
    ...
    async getProfile(userId: string): Promise<UserProfile | null> {
        try {
            const response = await fetch(`${cosmoAPI.getBaseUrl()}/api/profile/${userId}`);
            const data = await response.json();
            return data.success ? data.profile : null;
        } catch (error) {
    ...
    async acceptConsent(userId: string, dataCollectionConsent: boolean): Promise<boolean> {
        try {
            const response = await fetch(`${cosmoAPI.getBaseUrl()}/api/profile/${userId}/consent`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    async useTokens(userId: string, amount: number): Promise<boolean> {
        try {
            const response = await fetch(`${cosmoAPI.getBaseUrl()}/api/profile/${userId}/tokens/use`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
export const historyAPI = {
     ...
     */
    async getHistories(userId: string): Promise<RawChatHistory[]> {
        try {
            const response = await fetch(`${cosmoAPI.getBaseUrl()}/api/history/${userId}`);
            const data = await response.json();
    ...
    async createChat(userId: string, title: string, messages: any[]): Promise<string | null> {
        try {
            const response = await fetch(`${cosmoAPI.getBaseUrl()}/api/history`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
     ...
     * Update chat
     */
    async updateChat(chatId: string, updates: { title?: string; messages?: RawMessage[] }): Promise<boolean> {
        try {
            const response = await fetch(`${cosmoAPI.getBaseUrl()}/api/history/${chatId}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(updates),
            });
...
export const preferencesAPI = {
    ...
    async getPreferences(userId: string): Promise<UserPreferences | null> {
        try {
            const response = await fetch(`${cosmoAPI.getBaseUrl()}/api/profile/${userId}/preferences`);
            const data = await response.json();
    ...
    async updateNsfwPreference(userId: string, nsfwEnabled: boolean): Promise<boolean> {
        try {
            const response = await fetch(`${cosmoAPI.getBaseUrl()}/api/profile/${userId}/preferences/nsfw`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
    ...
    async updateHfModel(userId: string, model: string): Promise<boolean> {
        try {
            const response = await fetch(`${cosmoAPI.getBaseUrl()}/api/profile/${userId}/preferences/hf-model`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
    ...
    async updateHfApiKey(userId: string, apiKey: string): Promise<boolean> {
        try {
            const response = await fetch(`${cosmoAPI.getBaseUrl()}/api/profile/${userId}/preferences/hf-api-key`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
