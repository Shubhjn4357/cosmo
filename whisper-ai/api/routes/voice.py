...
async def transcribe_with_openai(audio_bytes: bytes, filename: str, model_id: str = "whisper-1") -> dict:
    import httpx
...
def transcribe_with_local_whisper(audio_bytes: bytes) -> dict:
    ...
    try:
        ...
        logger.error(f"Local Whisper error: {exc}")
...
async def _transcribe_audio_bytes(
    *,
    ...
    if stt_model_id == "local-whisper-base" or use_local_override or USE_LOCAL_WHISPER:
        result = await asyncio.to_thread(transcribe_with_local_whisper, audio_bytes)
        return {**result, "model_id": "local-whisper-base"}

    result = await transcribe_with_openai(audio_bytes, filename, model_id="whisper-1")
    return {**result, "model_id": "openai-whisper-1"}
...
async def _generate_text_response(
    *,
    ...
    if normalized_backend in {"self_learner", "self-learner", "whisper-micro-transformer"}:
        ...
        "text_backends": [
            {
                "id": "server",
                "name": "Server Runtime",
                "description": "Current Whisper server text runtime.",
            },
            {
                "id": "self_learner",
                "name": "Self Learner",
                "description": "Built-in Whisper scratch transformer with server fallback.",
            },
...
async def talk_audio(
    ...
    return Response(
        ...
        headers={
            ...
            "X-Whisper-Transcript": pipeline.transcript,
            "X-Whisper-Backend": pipeline.backend or pipeline.text_backend,
        },
    )
