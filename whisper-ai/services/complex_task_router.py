...
def complex_task_config() -> RuntimeConfig:
    return RuntimeConfig(
        ...
        model_id=os.getenv("WHISPER_COMPLEX_TASK_MODEL_ID", "Qwen/Qwen3-Coder-Next"),
        ...
        allow_remote_code=_env_flag("WHISPER_COMPLEX_TASK_TRUST_REMOTE_CODE", "true"),
        n_threads=int(os.getenv("WHISPER_COMPLEX_TASK_THREADS", os.getenv("LOCAL_MODEL_THREADS", "2"))),
    )
...
def prompt_complexity_score(message: str, *, history: list[dict[str, str]] | None = None) -> float:
    ...
    if len(re.findall(r"```|def |class |function |import |export |interface |pytest|traceback|stack", lowered)) >= 2:
        score += 0.28

    complex_terms = (
        "architecture", "refactor", "multi-file", "deep dive", "debug", "investigate",
        "agent", "plan", "implement", "optimize", "migration", "dataset", "training",
        "fine-tune", "pipeline", "evaluate", "benchmark", "reasoning",
    )
    score += sum(0.06 for term in complex_terms if term in lowered)
...
def should_use_complex_runtime(message: str, *, history: list[dict[str, str]] | None = None) -> bool:
    if not _env_flag("WHISPER_COMPLEX_TASK_ROUTING", "true"):
        return False
    return prompt_complexity_score(message, history=history) >= float(
        os.getenv("WHISPER_COMPLEX_TASK_THRESHOLD", "0.55")
    )
...
def generate_server_response(
    *,
    ...
    if should_use_complex_runtime(prompt, history=history):
