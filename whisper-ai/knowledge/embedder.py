...
class SentenceEmbedder:
    ...
    def __init__(self, model_name: str = "sentence-transformers/all-MiniLM-L6-v2", dim: int = 384):
        ...
        self.device = os.getenv("WHISPER_EMBEDDER_DEVICE", "cpu").strip() or "cpu"
        self.max_chars = max(128, int(os.getenv("WHISPER_EMBEDDER_MAX_CHARS", "1600")))
        self.max_seq_length = max(64, int(os.getenv("WHISPER_EMBEDDER_MAX_SEQ_LENGTH", "256")))
    ...
    def embed(self, texts: List[str]) -> np.ndarray:
        ...
        if self.use_model and self.model is not None:
            cached_vectors: list[np.ndarray | None] = [self._cache_get(text) for text in prepared]
            missing_pairs = [
                (index, text)
...
def get_embedder(
    ...
    requested_model = os.getenv("WHISPER_EMBEDDER_MODEL", model_name).strip() or model_name
    requested_dim = int(os.getenv("WHISPER_EMBEDDER_DIM", str(dim)))
    with _EMBEDDER_LOCK:
