/**
 * Whisper App - Task Queue Service
...
// Storage key
const TASK_QUEUE_KEY = 'Whisper_task_queue';
...
class TaskQueueService {
    ...
    private async loadQueue() {
        try {
            const saved = await AsyncStorage.getItem(TASK_QUEUE_KEY);
            if (saved) {
    ...
    private async saveQueue() {
        try {
            await AsyncStorage.setItem(TASK_QUEUE_KEY, JSON.stringify(this.queue));
        } catch (error) {
    ...
    private async executeChatTask(data: Record<string, any>) {
        const response = await whisperAPI.chat({
