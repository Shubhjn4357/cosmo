/**
 * Whisper App - Smart Mode API Service
...
export const smartModeAPI = {
    ...
    async chat(request: SmartChatRequest): Promise<SmartChatResponse> {
        try {
            const response = await fetch(`${whisperAPI.getBaseUrl()}/api/chat/smart`, {
                method: 'POST',
    ...
    async getStatus(): Promise<SmartModeStatus> {
        try {
            const response = await fetch(`${whisperAPI.getBaseUrl()}/api/chat/smart/status`);
            return await response.json();
        } catch (error) {
